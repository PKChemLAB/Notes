...
- 在给定的环境条件下，平衡态是系统的最稳定状态；
- 处于平衡态的系统，其所有强度状态函数在宏观水平上处处相等；
- 真正的平衡态必须包括环境在内：环境–系统平衡态。

相应地有三类平衡：力平衡 $(P,V)$、热平衡 $(T,S)$、化学平衡 $(\mu,n)$。

**唯象第二定律。热力学第二定律判据有两类形式：**
...
$$dS_{环} = \frac{dq_{可逆,环}}{T_{环}} = -\frac{dq}{T_{环}} = -\frac{dU-dw}{T_{环}}$$

$$dS \ge \frac{dq}{T_{环}}$$

- **环境的任何热都可以考虑为可逆热**：对环境，一切过程都近似为可逆过程：

$$\Delta S_{环} = \int_{始}^{终}\frac{-dq}{T_{环}} = \int_{始}^{终}\frac{-dU}{T_{环}} + \int_{始}^{终}\frac{dw}{T_{环}}$$

- 对系统，可以设计一个可逆过程：

$$\Delta S = \int_{始}^{终}\frac{dU}{T} + \int_{始}^{终}\frac{P\,dV}{T}$$

- 等容可逆：

$$\Delta S = \int_{始}^{终}\frac{dq^{等容可逆}}{T} = \int_{始}^{终}\frac{C_V\,dT}{T}$$

- 等压可逆：

$$\Delta S = \int_{始}^{终}\frac{dq^{等压可逆}}{T} = \int_{始}^{终}\frac{C_P\,dT}{T}$$

- 绝热过程：

$$dS \ge 0, \qquad dS_{环} \equiv 0$$
...
$$dS_{总} = dS = \left(\frac{1}{T_{环}} - \frac{1}{T}\right)d\theta$$

- 平衡态：

$$\left(\frac{\partial S_{总}}{\partial \theta}\right)_{V,n} = \frac{1}{T_{环}} - \frac{1}{T} = 0$$
...
$G$ 的意义：该状态在**定温定压**条件下的稳定度，稳定度越低，$G$ 越大。
...
能量最低原理可看作**环境熵变远远大于系统熵变**的情况，能量最低等同于环境熵最大。

---

### 3.5 广义化学平衡态

**定温定压具有特殊意义。** 定温定压条件涉及系统–环境之间和系统内部：

- 热平衡 $(T)$；
- 力平衡 $(P)$，集中于系统内进行分析（如系统内组分摩尔数 $n$ 变化）。
...
$$\left(\frac{\partial G}{\partial \xi_i}\right)_{T,P,\,\xi\ne\xi_i} = 0$$

- 偏离平衡：

$$\left(\frac{\partial G}{\partial \xi_i}\right)_{T,P,\,\xi\ne\xi_i} d\xi_i = dG > 0$$

$\xi\ne\xi_i$ 表示除 $\xi_i$ 外，保持其他独立摩尔数变量为常数。
